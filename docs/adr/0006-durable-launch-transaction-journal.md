...
事务不是锁的替代品：实际互斥由 ADR-0003 的 flock 提供；事务将文件系统、锁和进程证据连接起来，支持 daemon 启动或 CLI recover 的幂等决策。逻辑持久化数据模型与迁移策略见阶段 1C 的 [Data Model](../DATA_MODEL.md)；SQLite schema 与迁移实现仍待后续 POC。
