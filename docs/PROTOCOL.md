...
| -------- | --------------------------------------------------------------------------------------------------- |
| 状态     | 阶段 1C 冻结候选                                                                                    |
| 逻辑版本 | Protocol v1 Candidate（`1.0`）                                                                      |
...
本文件冻结 GUI、Tauri Host、`muxlaned` 与 `muxlane` CLI 之间的**逻辑控制协议**。它补充 [总体架构](ARCHITECTURE.md)、[Runtime 生命周期](RUNTIME_LIFECYCLE.md) 与 [恢复状态机](RECOVERY_STATE_MACHINE.md)，但不替代它们的锁、凭证或恢复不变量。
...
| 方法                            | purpose / caller                        | request model → response model                                          | 幂等性与副作用                                               | 能力 / 可能错误                                                                                         | 目标阶段 / POC                |
| ------------------------------- | --------------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- | ----------------------------- |
...
| `account.remove_eligibility`    | 判断是否可移除；Host、CLI               | AccountRef → Eligibility                                                | 只读；不删除                                                 | `account.archive.v1`; `ACCOUNT_IN_USE`, `CONFLICT`                                                      | 7；保留策略 POC               |
...
`assets.*` 与 `files.*` 仅保留为阶段 7 的未来命名空间；它们不在 Protocol v1 MVP 方法表中，也不授权任意文件访问。
