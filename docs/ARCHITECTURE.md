...
本文是阶段 1A 的总体架构冻结候选，描述目标设计，不代表 Account、Runtime、Daemon、Terminal 或协议已经实现。Muxlane 设计为本地优先、轻量、可恢复、可诊断的 Windows/WSL Codex Runtime 工作台：项目级隔离与凭证安全优先于完整 IDE 功能。目标是兼容经验证的最近稳定 Codex CLI 行为，而不固定未验证的内部 Schema。
...
Account Vault 目录为 `0700`，`auth.json` 为 `0600`，敏感文件只归当前 Linux 用户。共享目录只保存受治理的 Asset；Project Runtime、日志和活动凭证彼此隔离。事务、Recovery、锁和凭证目录需要在实际写入路径执行 fsync；SQLite 保存元数据、状态索引和事务引用，绝不保存 Token、Prompt 或原始 Terminal 内容。Project ID 必须稳定、随机且不从可泄露路径直接推导。删除进入 `archives/`，非立即物理删除。
...
凭证高层流程为 Account Vault → 原子 Credential Checkout → Runtime `auth.json` → Codex 运行/refresh → 原子 Credential Commit → Cleanup。正式实现要求临时文件、`0600`、fsync、同目录原子 rename、目录 fsync、持久事务、Hash、冲突保留和幂等 Recovery。详细生命周期与 Recovery 状态机留给阶段 1B 的 `RUNTIME_LIFECYCLE.md` 和 `RECOVERY_STATE_MACHINE.md`。
...
阶段 2：Project Runtime POC；阶段 3：Vault/Lock/Windows-WSL Bridge POC；阶段 4：Launch Transaction 与 Recovery POC；阶段 5：正式后台、CLI、Terminal；阶段 6：GUI 与 Usage；阶段 7：工作台与 Asset；阶段 8：发布、升级、运维。

待验证风险包括 Windows-WSL 本地桥接、`tmux` Control Mode、Codex CLI/App Server Schema 演进、`auth.json` refresh、PID 重用、WSL 重启、SQLite Migration、fsync、跨文件系统路径规范化、Tauri Capability、WebView Terminal 输入安全和诊断包泄露。
