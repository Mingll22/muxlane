...
| 命令/动作                                                                  | 平台            | 退出码 | 结果                                                |
| -------------------------------------------------------------------------- | --------------- | -----: | --------------------------------------------------- |
| `pnpm verify`                                                              | WSL             |      0 | PASS；前端 3 files / 4 tests，Rust WSL 集成 4 tests |
...
| `cargo test -p muxlaned -p muxlane-protocol --all-features -- --nocapture` | WSL             |      0 | PASS；1 protocol + 5 unit + 4 tmux integration      |
...
- bootstrap/reconnect 共 3 个新 stream，分别观察 21、33、45 个连续 tick，总计 99；missing=0、duplicate=0、reordered=0、cross_stream=0；
- 每个 stream 的 event sequence 都从 0 连续增长；旧 stream 输入返回 `stale_stream`；
- 512 条 burst：missing=0、duplicate=0、reordered=0、overflow=0；
- 4-test tmux 集成套件曾连续运行 5 次，全部通过；
