...
当前仓库包含阶段 0 工程基础、阶段 1 冻结设计、阶段 2/3 POC，以及 Phase 4/5 正式 WSL Runtime Control Plane。Phase 4/5 的本地 Linux、隔离 WSL、Windows MSVC/Tauri 与跨边界验收已完成；阶段是否最终关闭仍以 PR review/CI、squash merge 和合并后 main CI 为准。
...
Phase 4/5 本地实现与验收结论为 `PASS`，正在执行 PR/CI/Git 阶段收口。PR、main merge 和合并后 CI 全绿前不得进入 Phase 6。

### 后续粗粒度路线

1. 完成 Phase 4/5 PR review、CI、squash merge、合并后 main CI 和测试环境清理。
2. 门禁全部关闭后进入 Phase 6 GUI；不提前进入 Skills/MCP/工作台或发布阶段。
...
- GitHub PR review/CI、post-merge main CI 和阶段分支删除仍待本次阶段收口完成。
...
完成 PR/CI/merge/main CI；全绿后才允许启动 Phase 6。
...
- 工作区：Phase 4/5 本地实现与证据提交完成；具体 HEAD 与清洁状态以当前 Git 为准。
- 关键验证：`pnpm verify`、Rust fault matrix、正式 control/Terminal integration、专用 WSL terminate、Recovery 二次 kill、systemd-nspawn boot change、Windows MSVC/Tauri 与 Windows→WSL smoke 均通过；真实 Account Usage `NOT RUN`；PR/CI 状态以当前 GitHub 为准。
