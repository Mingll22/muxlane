...
| 工作台配置      | SQLite schema v5 的 settings/templates/presets                   | GUI 当前表单                           | localStorage、仓库配置       |
| 输入历史        | SQLite schema v5 `input_history`                                 | GUI 搜索结果                           | Terminal 输出、Codex Session |
| 文件预览        | daemon 每次在 canonical Project root 下重新解析的普通文本文件    | GUI 临时预览                           | GUI 拼接路径、symlink target |
...
| `template_id`          | 非秘密 Project template 的稳定 ID。                                                                                                                                                |
| `preset_id`            | Project command preset 的稳定 ID。                                                                                                                                                 |
| `history_id`           | 明确提交的一条 Prompt/Shell 输入记录 ID。                                                                                                                                          |
...
### 4.7 ProjectSettings、ProjectTemplate、CommandPreset 与 InputHistory

`ProjectSettings` 保存 `project_id`、默认 Account、模型、Reasoning 与更新时间；`ProjectTemplate` 保存非秘密默认模型、Reasoning、Terminal preset 和 command preset；`CommandPreset` 归属一个 Project，保存名称、说明、terminal kind、受控工作目录和命令文本。Preset 只能由用户明确点击填入或执行，不能作为仓库启动钩子。

`InputHistory` 保存 `history_id`、`project_id`、可选 `terminal_id`/`thread_id`、`kind`、`input_text` 与 `created_at`。它只记录 GUI 明确提交的 Prompt/Shell 输入，不记录 Terminal 输出，不改写 Codex 原生 Session；秘密 marker 和超限输入被拒绝，诊断包默认排除。

Skills、MCP、Plugins、统一 Asset/ProjectAsset、CodeMirror 和文件写入由 ADR-0013 延期，不属于 schema v5 或当前协议合同。
...
erDiagram
  ...
  PROJECT ||--o{ COMMAND_PRESET : offers
  PROJECT ||--o{ INPUT_HISTORY : retains_locally
  TERMINAL ||--o{ INPUT_HISTORY : optionally_associates
  PROJECT_TEMPLATE ||--o{ PROJECT : initializes
```
...
3. Account 和 Project 采用归档/软删除，不物理删除历史记录。
...
| Template/Preset      | 仅保存非秘密配置；用户可显式复制、应用或删除。                                                                       |
| InputHistory         | 可清理单项或 Project 历史；不触碰 Codex 原生 Session。                                                               |
...
`schema_version`、migration history 与迁移锁属于 SQLite 的逻辑职责。当前 schema v5 增加 `project_settings`、`project_templates`、`command_presets` 和 `input_history`。迁移采用向前迁移：迁移前执行健康检查和备份；迁移期间禁止业务写操作；完成后才允许新 Schema 的写请求。
...
- Terminal order by Project；Project/Account archived status；template/preset/history queries。
...
| Public project metadata  | 项目显示名、非秘密 template/preset 名称                                            | 可显示，但不应借此推断本地秘密。                                |
...
用户可在 GUI 清理单项或整个 Project 输入历史；本地历史仍不得被解释为可公开导出，诊断默认排除其内容。
