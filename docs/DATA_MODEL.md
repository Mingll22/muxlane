...
`RecoveryIncident`：`incident_id`、`transaction_id`、`classification`、`detected_at`、`status`、`requires_user_action`、`evidence_references`、`resolution_summary`、`resolved_by_action`、`resolved_at`。
...
人工修复或重试会创建新的 RecoveryAttempt（必要时新的 incident），并把 outcome 记录为 `safe_cleanup_completed`、`conflict_preserved`、`failed` 等；它**不会**修改旧 Transaction 的终态。若用户选择发起新的受管运行，则创建新的 Launch/Transaction。这样 `recovered` 始终表示无冲突地完成原 Transaction 的自动或崩溃收尾，而不会掩盖此前的 `failed` 历史。
...
| 对象                 | 规则                                                                                                       |
| -------------------- | ---------------------------------------------------------------------------------------------------------- |
| Account              | 只能在无活动 Account Lock、无活动 Launch、无阻断 Recovery/凭证事务时归档；凭证的物理处置需要后续明确流程。 |
| Project              | 只逻辑归档；先确认无运行、无 Runtime `auth.json`、无待签回事务与真实活动锁。                               |
