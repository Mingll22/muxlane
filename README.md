...
> **Pre-alpha：阶段 1 的需求与架构已冻结；阶段 2 Runtime POC 与阶段 3 Terminal POC 已完成验证。正式业务能力尚未实现，不能作为正式工具使用。**
...
阶段 1 已冻结需求、总体架构、威胁模型、Runtime 生命周期、恢复状态机、逻辑协议、数据模型、兼容策略和 ADR-0001～0012。阶段 2/3 POC 已验证有限 Runtime 与持久终端假设，但不代表生产能力已经实现；后续 POC 若得出相反证据，必须通过新的 ADR 修订。Phase 4 及之后的锁、事务、Crash Recovery、正式 daemon、SQLite 和产品能力尚未开始。
